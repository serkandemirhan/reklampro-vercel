...
export default function JobDetailPage({ params }: { params: { id: string } }) {
  ...
  const loadJob = useCallback(async () => {
    setJobLoading(true)
    try {
      const r = await fetch(`/api/jobs/${jobId}/detail`, { cache: 'no-store' })
      if (r.ok) {
  ...
  const statusPretty = useMemo(
    () => (s: string) =>
      ({ not_started: 'Başlamadı', in_progress: 'Devam', paused: 'Durduruldu', completed: 'Bitti', canceled: 'İptal', frozen: 'Donduruldu' } as any)[s] ?? s,
    []
  )
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link href={`/jobs/${jobId}/tasks`} className="btn btn-sm">Görev ekranını aç</Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <label className="form-control">
          ...
          <input className="input input-bordered" value={job?.title ?? ''} readOnly />
        </label>
        <label className="form-control">
          <span className="label-text">Durum</span>
          <input className="input input-bordered" value={statusPretty(job?.status ?? '')} readOnly />
        </label>
        <label className="form-control md:col-span-2">
          <span className="label-text">Açıklama</span>
          <textarea className="textarea textarea-bordered h-24" value={job?.description ?? ''} readOnly />
        </label>
        <label className="form-control">
          <span className="label-text">Müşteri</span>
          <input className="input input-bordered" value={job?.customer_name ?? ''} readOnly />
        </label>
        <label className="form-control">
          <span className="label-text">Oluşturma</span>
          <input className="input input-bordered" value={job?.created_at ?? ''} readOnly />
        </label>
        {jobLoading && <div className="md:col-span-2 text-sm text-gray-500">İş bilgisi yükleniyor…</div>}
      </div>
      ...
      <section className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <button className="btn btn-sm" onClick={() => { loadSteps(); loadJob(); }} disabled={loading || jobLoading}>
            {(loading || jobLoading) ? 'Yükleniyor…' : 'Yenile'}
          </button>
